...
//echo "jalan";
  ...
  $urlTemp =  'https://simsdm.setneg.go.id/proses/integrasi/wsremunerasi.php/'.$periode.'/'.$tahun.'/remun@dktu99';
 //$urlTemp = '012016.txt';
  //echo $urlTemp;
...
foreach( $jsonRemun as $i){
  //echo fnDecrypt($i['NamaLengkap'],$sSecretKey)."=>".fnDecrypt($i['UM'],$sSecretKey)."<br>";
    ...
    $sql_data = "insert into taKaryawanTmp_json (NIP,NAMA,TMT,KODE_INSTANSI,KODE_UNIT,KODE_DEPUTI,KODE_BIRO,KODE_BAGIAN,KODE_SUBBAGIAN,KODE_GOLONGAN,KODE_GRADE,KODE_ESELON,NIP_BARU,JABATAN,NO_URUT,KET,NPWP,UM,
	TGL_PENSIUN,JPEG,STRUKTUR_BARU) VALUES ('".fnDecrypt($i['NIPLama'],$sSecretKey)."','".str_replace('\'','\'\'',fnDecrypt($i['NamaLengkap'],$sSecretKey))."',
	'".date('Y-m-d',  strtotime(fnDecrypt($i['TMTCPNS'],$sSecretKey)))."','".fnDecrypt($i['IdInstansi'],$sSecretKey)."','".fnDecrypt($i['IdUnit'],$sSecretKey)."',
	'".fnDecrypt($i['IdDeputi'],$sSecretKey)."','".fnDecrypt($i['IdBiro'],$sSecretKey)."','".fnDecrypt($i['IdBagian'],$sSecretKey)."','".fnDecrypt($i['IdSubbagian'],$sSecretKey)."',
	'".fnDecrypt($i['KdGol'],$sSecretKey)."','".fnDecrypt($i['IdClass'],$sSecretKey)."','".fnDecrypt($i['KdEselon'],$sSecretKey)."','".fnDecrypt($i['NIPBaru'],$sSecretKey)."',
	'".fnDecrypt($i['Jabatan'],$sSecretKey)."','".fnDecrypt($i['Urut'],$sSecretKey)."','".fnDecrypt($i['Ket'],$sSecretKey)."','".fnDecrypt($i['NPWP'],$sSecretKey)."',
	'".fnDecrypt($i['UM'],$sSecretKey)."','".date('Y-m-d',strtotime(fnDecrypt($i['BUP'],$sSecretKey)))."','".fnDecrypt($i['JenisPegawai'],$sSecretKey)."','".fnDecrypt($i['StrukturBaru'],$sSecretKey)."')";
 //echo $sql_data;
...
echo '<script language="javascript">document.getElementById("information").innerHTML="'.$total.' Row completed"</script>';
	
	
	//}
	//END OF READABLE URL
	/*else{
		echo "Data Karyawan Belum Tersedia";	

	}*/
  
 
 //membersihkan table taKaryawanTMP
