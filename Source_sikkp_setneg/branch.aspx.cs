...
//        Nama file     : branch.aspx.cs                             (Pseudo code)
//        File terkait  : branch.aspx                                (Form)
//                        \App_LocalResources\branch.aspx.id.resx    (Display label bahasa Indonesia)
//                        \App_LocalResources\branch.aspx.en.resx    (Display label bahasa Inggris)
...
namespace rtwin
	{
		public partial class branch : BasePage
		{
            ...
            {
                GvBranch.Visible = show;
                FvBranch.Visible = !show;
                if (show)
                {
                    FvBranch.DefaultMode = FormViewMode.ReadOnly;
                }
            }
            ...
            protected void Page_Load(object sender, System.EventArgs e)
			{
                ...
                else
                {
                    ...
                    if ((!Page.IsPostBack) && (Session["GradeID"].ToString() != "1"))
                    {
                        ...
                        GvBranch.Columns[3].Visible = false;
                    }
                }
			}
			
            //fungsi: mengatur state Gridview setelah berhasil bound data
			protected void GvBranch_DataBound(object sender, System.EventArgs e)
			{
                //seleksi jumlah data kosong atau tidak
                if (GvBranch.Rows.Count > 0)
                {
                    //Data tidak kosong
                    GridViewRow rowPager = GvBranch.BottomPagerRow;
                    ...
                    if (rowPager != null)
                    {
                        ...
                        {
                            GvBranch.PageIndex = GvBranch.PageCount - 1;
                        }
                        ...
                        txtHalaman.Text = Convert.ToString(GvBranch.PageIndex + 1);
                        lblJumlahHalaman.Text = GvBranch.PageCount.ToString();
                    }
                    showGridView(true);
                }
                else
            ...
            protected void lnkBtnInsert_Click(object sender, EventArgs e)
            {
                if (Page.IsValid)
                {
                    ...
                    if (GvBranch.Rows.Count > 0)
                    {
                        //Data tidak kosong
                        sendToLastPage = true;
                        GvBranch.ShowFooter = true;
                        this.ScriptManager1.SetFocus(GvBranch.FooterRow.Cells[0].FindControl("txtCol0Footer").ClientID);
                    }
                    else
                    {
                        //Data Kosong
                        FvBranch.DefaultMode = FormViewMode.Insert;
                        this.ScriptManager1.SetFocus(FvBranch.FindControl("txtCol0FormView"));
                    }
                }
            }
            ...
            protected void FvBranch_ItemCommand(object sender, FormViewCommandEventArgs e)
            {
            ...
            protected void dsCabang_Inserting(object sender, SqlDataSourceCommandEventArgs e)
            {
                ...
                if (GvBranch.Rows.Count > 0)
                {
                    TextBox NewKode = (TextBox)GvBranch.FooterRow.FindControl("txtCol0Footer");
                    TextBox NewNamaCabang = (TextBox)GvBranch.FooterRow.FindControl("txtCol1Footer");

                    e.Command.Parameters["@kode_cabang"].Value = NewKode.Text;
                    e.Command.Parameters["@nama_cabang"].Value = NewNamaCabang.Text;
                }
            }
            ...
            protected void dsCabang_Inserted(object sender, SqlDataSourceStatusEventArgs e)
            {
                ...
                else
                {
                    ...
                    azlib.AddUserAct(Session["UserID"].ToString(), "1211", e.Command.Parameters["@KODE_CABANG"].Value.ToString(), Application["strCn"].ToString());
                }
                GvBranch.ShowFooter = false;
                showGridView(true);
            }
            ...
            protected void dsCabang_Deleted(object sender, SqlDataSourceStatusEventArgs e)
            {
                ...
                {
                    //Tidak terjadi Error, lakukan penulisan log/userAct ke database
                    azlib.AddUserAct(Session["UserID"].ToString(), "1213", e.Command.Parameters["@KODE_CABANG"].Value.ToString(), Application["strCn"].ToString());
                }
            }
            ...
            protected void dsCabang_Updated(object sender, SqlDataSourceStatusEventArgs e)
            {
                ...
                {
                    //Tidak terjadi Error, lakukan penulisan log/userAct ke database
                    azlib.AddUserAct(Session["UserID"].ToString(), "1212", e.Command.Parameters["@KODE_CABANG"].Value.ToString(), Application["strCn"].ToString());
                }
            }
            ...
            protected void txtHalaman_TextChanged(object sender, System.EventArgs e)
            {
                ...
                try
                {
                    if (int.Parse(txtHalaman.Text) <= GvBranch.PageCount + 1 && int.Parse(txtHalaman.Text) > 0)
                    {
                        GvBranch.PageIndex = (int.Parse(txtHalaman.Text)) - 1;
                        GvBranch.DataBind();
                    }
                }
                catch (Exception)
                {
                    txtHalaman.Text = (GvBranch.PageIndex + 1).ToString();
                }
