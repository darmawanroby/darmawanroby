		...
		public partial class summary_report : BasePage
		{
			...
			protected void Page_Load(object sender, EventArgs e)
			{
				...
				else
				{
					...
					strfilerpt = "app_data/";
					if ((Convert.ToInt32(Session["strTglTxt"].ToString().Substring(Session["strTglTxt"].ToString().Length - 4)) < 2014) && (Session["strKodeRpt"].ToString() != "I3"))
					{
					strfilerpt += "siapdb/";
					}
                ...
                if (rptOpen)
				{
					...
					crys.Load(Server.MapPath(strfilerpt));
                        if ((Convert.ToInt32(Session["strTglTxt"].ToString().Substring(Session["strTglTxt"].ToString().Length - 4)) >= 2014) ||(Session["strKodeRpt"].ToString() == "I3") ||(Session["strKodeRpt"].ToString() == "I4"))
                        {
                    crys.SetDatabaseLogon("rekabio", "Meinardi", Application["DatabaseServer"].ToString(), "DataReal");
                        }
                        else
                        {
                    crys.SetDatabaseLogon("rekabio", "Meinardi", "siapdb", "DataReal");
					crys.VerifyDatabase();
                        }
                    ...
                    if (strtiperpt == "4")
                    {
                        ...
                        {
                            q = "select " + strkolomrpt + " from q_LambatCepatPulang WHERE TAHUN = '" + Session["strTgl"].ToString().Substring(0, 4) + "'" + Session["strFilter"] + " ORDER BY JML_LAMBAT_CPTPLG," + Session["strOrder"];
                        }
                        else if (Session["strKodeRpt"].ToString() == "I2")
                        {
                            q = "select " + strkolomrpt + " from q_Pelanggaran WHERE PERIODE_PELANGGARAN = '" + Session["strTgl"].ToString() + "'" + Session["strFilter"] + " ORDER BY JML_MANGKIR," + Session["strOrder"];
                        }
                        //else if (Session["strKodeRpt"].ToString() == "I3")
                        //{
                        //    q = "select " + strkolomrpt + " from q_Pelanggaran_dan_Akumulasi2 WHERE PERIODE_PELANGGARAN = '" + Session["strTgl"].ToString() + "' ORDER BY JML_MANGKIR," + Session["strOrder"];
                        //}
                        //Response.Write(q);
                        ...
                        if (Session["strKodeRpt"].ToString() == "I1")
					    {
							daRpt.Fill(ds, "q_LambatCepatPulang");
                        }
                        else if (Session["strKodeRpt"].ToString() == "I2")
					    {
							daRpt.Fill(ds, "q_Pelanggaran");
                        }
                        else
					    {
                    ...
                    if (Session["strKodeRpt"].ToString().Substring(0, 1) != "H")
                    {
                        ...
                        if ((Session["strKodeRpt"].ToString() == "D1") || (Session["strKodeRpt"].ToString() == "D9"))
                        {
