...
//        Nama file     : detail.aspx.cs                             (Pseudo code)

//        File terkait  : detail.aspx                                (Form)
//                        \App_LocalResources\detail.aspx.id.resx    (Display label bahasa Indonesia)
//                        \App_LocalResources\detail.aspx.en.resx    (Display label bahasa Inggris)
//                        \App_Code\azlib.cs                         (Kumpulan fungsi-fungsi)
//
//
//        Deskripsi     : Halaman untuk menampilkan filter laporan bulanan
	...
	namespace rtwin
	{
		...
		public partial class detail : BasePage
		{
            public void setComponentFilter()
            {
                ...
                {
                    radTanggal.Visible = true;
                    txtFilterItem0.Visible = true;
                    imgFilterItem0.Visible = true;
                    lblTo.Visible = true;
                    txtFilterItem1.Visible = true;
                    imgFilterItem1.Visible = true;
                }
            }
            ...
            //fungsi: menampilkan tipe-tipe laporan dan tipe-tipe ijin
            public void setKodeIjin()
			{
				SqlConnection cn = new SqlConnection(Application["strCn"].ToString());
				string strQuery = "";

                strQuery = "SELECT KODE_LAPORAN, NAMA_LAPORAN AS NAMA_LAPORAN FROM taLaporan WHERE PERIODE_LAPORAN='1'";
				...
				strQuery = "SELECT KODE_IJIN, KODE_IJIN + ': ' + KET_IJIN AS KETERANGAN_IJIN FROM taIjin where kode_tidak_hadir = '1'";
			...
			protected void Page_Load(object sender, EventArgs e)
			{
				...
				if (! Page.IsPostBack)
				{
					...
					byte j;
					for (j = 0; j <= cblKriteria.Items.Count - 1; j++)
					{
						cblKriteria.Items[j].Selected = true;
					}
					for (j = 0; j <= cblKodeIjin.Items.Count - 1; j++)
					{
						cblKodeIjin.Items[j].Selected = true;
					}
			...
			protected void btnShow_Click(object sender, EventArgs e)
			{
                if (Page.IsValid)
                {
                    ...
                    if ((ddlBiro.Items.Count > 0) && (ddlBiro.SelectedValue != "0000"))
                    {
                        ...
                        if ((ddlBiro.SelectedValue == "0141") && (rblReportType.SelectedValue.Substring(0, 1) == "E")) 
                        {
							strFilter += " AND (KODE_BIRO='" + ddlBiro.SelectedValue + "' OR NIP IN ('196111291986031002','060048850'))";
                        }
                        else
                        {                     
                    ...
                    if ((Session["GradeID"].ToString().Substring(0, 1) == "2")/* || (Session["GradeID"].ToString() == "1a")*/)
                    {
                    ...
                    else if (radBulan.Checked)
                    {
                        if (azlib.IsNumeric(ddlTahun.SelectedValue.ToString()))
                        {
                            ...
                            if (rblReportType.SelectedValue.Substring(0, 1) != "B") // rekap
                            {
                    ...
                    //{
                    //ExceptionDetails.Text = strFilter;
                    ////Response.Write(strFilter);
                    //} else {
                    Response.Redirect("detail_report.aspx");
                    //}
/*
                    //==================================== WINDOW POPUP =======================================
                    ...
                    sb.Append("window.open('detail_report2.aspx', 'CustomPopUp');<");
                    sb.Append("/script>");
...
*/
            ...
            {
                Response.Redirect("upload_overtime_report.aspx");
            }
}
	}
