<%@ Page Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="ceremony_manual.aspx.cs" Inherits="rtwin.ceremony_manual" title="Sistem Kehadiran Kerja Pegawai Kementerian Sekretariat Negara Republik Indonesia" Culture="auto:id-ID" UICulture="auto" %>
...
</asp:GridView><asp:SqlDataSource id="dsceremony_manual" runat="server" SelectCommand="SELECT NIP, PIN, NAMA, TGL_MASUK, KODE_RANGE, JAM_MASUK, JAM_KELUAR FROM q_AbsenUpacara" ConnectionString="<%$ ConnectionStrings:strCn %>" UpdateCommand="UPDATE taTmpAbsenUpacara SET JAM_MASUK = @JAM_MASUK, JAM_KELUAR = @JAM_KELUAR, ISI_MANUAL = 'Y' WHERE (NIP = @NIP) AND (TGL_MASUK = @TGL_MASUK)" InsertCommand="UPDATE taTmpAbsenUpacara SET JAM_MASUK = @JAM_MASUK, JAM_KELUAR = @JAM_KELUAR, ISI_MANUAL = 'Y' WHERE (NIP = @NIP) AND (TGL_MASUK = @TGL_MASUK)" OnDeleted="dsceremony_manual_Deleted" DeleteCommand="UPDATE taTmpAbsenUpacara SET JAM_MASUK = NULL, JAM_KELUAR = NULL, ISI_MANUAL = NULL WHERE (NIP = @NIP) AND (TGL_MASUK = @TGL_MASUK)" OnUpdating="dsceremony_manualEdit_Updating" OnUpdated="dsceremony_manualEdit_Updated" OnInserting="dsceremony_manualEdit_Inserting" OnInserted="dsceremony_manualEdit_Inserted"><DeleteParameters>
...
<asp:Parameter Name="TGL_MASUK"></asp:Parameter>
                ...
                </asp:SqlDataSource><asp:FormView id="Fvceremony_manual" runat="server" CssClass="FormView" DataSourceID="dsceremony_manual" OnDataBound="Fvceremony_manual_DataBound" DataKeyNames="NIP,PIN,TGL_MASUK" OnItemCommand="Fvceremony_manual_ItemCommand"><EditItemTemplate>
