...
namespace rtwin
	{
		public partial class connection : BasePage
		{
            ...
            protected void Page_Load(object sender, System.EventArgs e)
			{
                ...
                else
                {
                    ...
                    if (!Page.IsPostBack)
                    {
                        ...
                        //GvConnection.Columns[2].Visible = false;
                        //GvConnection.Columns[3].Visible = false;
                    }
                    }
                }
			}
			...
			protected void GvConnection_DataBound(object sender, System.EventArgs e)
			{
                //seleksi jumlah data kosong atau tidak
                if (GvConnection.Rows.Count > 0)
                {
                    //Data tidak kosong
                    GridViewRow rowPager = GvConnection.BottomPagerRow;
                    ...
                    if (rowPager != null)
                    {
                        ...
                        {
                            GvConnection.PageIndex = GvConnection.PageCount - 1;
                        }
                        ...
                        txtHalaman.Text = Convert.ToString(GvConnection.PageIndex + 1);
                        lblJumlahHalaman.Text = GvConnection.PageCount.ToString();
                    }
                    showGridView(true);
                }
                else
            ...
            protected void GvConnection_RowCommand(object sender, GridViewCommandEventArgs e)
            {
                ...
                {
                    GvConnection.ShowFooter = false;
                }
            }

            //fungsi: mengatur state dari setiap Row pada Gridview
            protected void GvConnection_RowDataBound(object sender, GridViewRowEventArgs e)
            {
            ...
            protected void GvConnection_RowEditing(object sender, GridViewEditEventArgs e)
            {
                if (Page.IsValid)
                {
                    ...
                    this.ScriptManager1.SetFocus(GvConnection.Rows[e.NewEditIndex].Cells[1].FindControl("txtCol1Edit").ClientID);
                }
                else
                {
                    e.Cancel = true;
            ...
            protected void lnkBtnInsert_Click(object sender, EventArgs e)
            {
                if (Page.IsValid)
                {
                    ...
                    if (GvConnection.Rows.Count > 0)
                    {
                        //Data tidak kosong
                        sendToLastPage = true;
                        GvConnection.ShowFooter = true;
                        this.ScriptManager1.SetFocus(GvConnection.FooterRow.Cells[0].FindControl("txtCol0Footer").ClientID);
                    }
                    else
                    {
                        //Data Kosong
            ...
            protected void dsConnection_Inserting(object sender, SqlDataSourceCommandEventArgs e)
            {
                ...
                if (GvConnection.Rows.Count > 0)
                {
                    TextBox NoMesin = (TextBox)GvConnection.FooterRow.FindControl("txtCol0Footer");
                    TextBox NamaMesin = (TextBox)GvConnection.FooterRow.FindControl("txtCol1Footer");
                    DropDownList TipeKoneksi = (DropDownList)GvConnection.FooterRow.FindControl("DDListCol2Footer");
                    TextBox AlamatMesin = (TextBox)GvConnection.FooterRow.FindControl("txtCol3Footer");
                    TextBox NoReader = (TextBox)GvConnection.FooterRow.FindControl("txtCol4Footer");
                    DropDownList ModeMesin = (DropDownList)GvConnection.FooterRow.FindControl("DDListCol5Footer");
                    DropDownList KodeLokasi = (DropDownList)GvConnection.FooterRow.FindControl("DDListCol7Footer");


                    e.Command.Parameters["@NO_MESIN"].Value = NoMesin.Text;
                    e.Command.Parameters["@NAMA_MESIN"].Value = NamaMesin.Text;
                    e.Command.Parameters["@TIPE_KONEKSI"].Value = TipeKoneksi.Text;
                    e.Command.Parameters["@ALAMAT_MESIN"].Value = AlamatMesin.Text;
                    e.Command.Parameters["@NO_READER"].Value = NoReader.Text;
                    e.Command.Parameters["@MODE_MESIN"].Value = ModeMesin.Text;
                    e.Command.Parameters["@KODE_LOKASI"].Value = KodeLokasi.Text;

                }
            }

            //fungsi: menghandle ketika datasource telah selesai melakukan proses insert
            protected void dsConnection_Inserted(object sender, SqlDataSourceStatusEventArgs e)
            {
                ...
                else
                {
                    ...
                    azlib.AddUserAct(Session["UserID"].ToString(), "1221", e.Command.Parameters["@KODE_LOKASI"].Value.ToString(), Application["strCn"].ToString());
                }
                GvConnection.ShowFooter = false;
                showGridView(true);
            }
            ...
            protected void dsConnection_Deleted(object sender, SqlDataSourceStatusEventArgs e)
            {
                //seleksi apakah ada error (ditandai dengan jumlah baris yg terkena efek, 
                //jika = 0 / tidak ada yg terkena efek maka terjadi error)
                //jika tidak ada error / affectedRow > 0 maka lakukan penulisan log/userAct ke database
                ...
                {
                    //Tidak terjadi Error, lakukan penulisan log/userAct ke database
                    azlib.AddUserAct(Session["UserID"].ToString(), "1223", e.Command.Parameters["@NO_"].Value.ToString(), Application["strCn"].ToString());
                }
            }

            //fungsi: menghandle ketika datasource telah selesai melakukan proses update
            protected void dsConnection_Updated(object sender, SqlDataSourceStatusEventArgs e)
            {
                //seleksi apakah ada error (ditandai dengan jumlah baris yg terkena efek, 
                //jika = 0 / tidak ada yg terkena efek maka terjadi error)
                //jika tidak ada error / affectedRow > 0 maka lakukan penulisan log/userAct ke database
                ...
                {
                    //Tidak terjadi Error, lakukan penulisan log/userAct ke database
                    azlib.AddUserAct(Session["UserID"].ToString(), "1222", e.Command.Parameters["@NO_MESIN"].Value.ToString(), Application["strCn"].ToString());
                }
            }
            ...
            protected void txtHalaman_TextChanged(object sender, System.EventArgs e)
            {
                ...
                try
                {
                    if (int.Parse(txtHalaman.Text) <= GvConnection.PageCount + 1 && int.Parse(txtHalaman.Text) > 0)
                    {
                        GvConnection.PageIndex = (int.Parse(txtHalaman.Text)) - 1;
                        GvConnection.DataBind();
                    }
                }
                catch (Exception)
                {
                    txtHalaman.Text = (GvConnection.PageIndex + 1).ToString();
                }
            }
}
}
