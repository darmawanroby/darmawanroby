...
//        Nama file     : punishment.aspx.cs                             (Pseudo code)

//        File terkait  : punishment.aspx                                (Form)
//                        \App_LocalResources\punishment.aspx.id.resx    (Display label bahasa Indonesia)
//                        \App_LocalResources\punishment.aspx.en.resx    (Display label bahasa Inggris)
        ...
        public partial class change_shift : BasePage
		{
            ...
            private void doFiltering()
            {
                try
                {
                    ...
                    dsPunishment.SelectCommand += strFilter + " ORDER BY TGL_AWAL ASC";
                }
                catch (Exception ex)
                { }
            }
            ...
            private void setComponent()
            {
                if ((Session["GradeID"].ToString() != "1") && (Session["GradeID"].ToString() != "2") && (Session["GradeID"].ToString() != "3"))
                {
                    ...
                    GvPunishment.Columns[5].Visible = false;
                    GvPunishment.Columns[6].Visible = false;
                }
                BoundField Column0 = (BoundField)GvPunishment.Columns[0];
                if (isEntryUseNIP())
                {
			...
			{
                //if ((Session["CabangID"].ToString() == "003") && (Session["GradeID"].ToString() != "1"))
                //{
				//	Response.Redirect("foul.aspx");
                //}
