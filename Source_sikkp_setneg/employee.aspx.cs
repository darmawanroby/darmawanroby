	...
	namespace rtwin
	{
		public partial class employee : BasePage
		{
            ...
            private void doFiltering()
            {
                ...
                {
                    strFilter += " WHERE (NIP = '" + txtFilterItem0.Text.Replace("'", "''") + "')";
                }
			...
			protected void Page_Load(object sender, EventArgs e)
			{
                ...
                ScriptManager.GetCurrent(this).RegisterPostBackControl(FvKaryawan);
            ...
            protected void lblTambah_Click(object sender, EventArgs e)
            {
                if (Page.IsValid)
                {
	                ...
	                //if ((Session["GradeID"].ToString() == "2") || (Session["GradeID"].ToString() == "3"))
					//{
		            //    DropDownList ddlStatusPegawai = (DropDownList)FvKaryawan.FindControl("ddlStatusPegawai");
					//	dsStatusPegawai.SelectCommand = "select * from taStatusPegawai where kode_status_pegawai in ('0','5','6')";
					//	ddlStatusPegawai.DataBind();
					//}
                }
            }
            ...
            protected void GvKaryawan_RowEditing(object sender, System.Web.UI.WebControls.GridViewEditEventArgs e)
            {
                if (Page.IsValid)
                {
                    dsKaryawanManajemen.FilterExpression = "NIP = '" + ((Label)GvKaryawan.Rows[e.NewEditIndex].Cells[0].FindControl("lblCol0Item")).Text + "'";
                    e.Cancel = true;
            ...
            protected void GvKaryawan_RowCommand(object sender, GridViewCommandEventArgs e)
            {
                if (e.CommandName == "Detail")
                {
                    if (Page.IsValid)
                    {
                        ...
                        dsKaryawanManajemen.FilterExpression = "NIP = '" + ((Label)((GridViewRow)((ImageButton)e.CommandSource).NamingContainer).Cells[0].FindControl("lblCol0Item")).Text + "'";
                        showGridView(false);
                    }
                }
            }
            ...
            protected void FvKaryawan_ItemCommand(object sender, FormViewCommandEventArgs e)
            {
                ...
                else if (e.CommandName == "Edit_Detail")
                {
                    if (Page.IsValid)
                    {
                        dsKaryawanManajemen.FilterExpression = "NIP = '" + ((Label)FvKaryawan.FindControl("lblNIP")).Text + "'";
                        FvKaryawan.DefaultMode = FormViewMode.Edit;
                        FvKaryawan.ChangeMode(FormViewMode.Edit);
                    }
                }
...
//                }
                ...
                else if (Session["GradeID"].ToString() == "2")
                {
                    if (FvKaryawan.CurrentMode == FormViewMode.Edit)
                    {
	                    if ((((DropDownList)FvKaryawan.FindControl("ddlStatusPegawai")).SelectedValue == "5") || (((DropDownList)FvKaryawan.FindControl("ddlStatusPegawai")).SelectedValue == "6"))
		                {
			                ((DropDownList)FvKaryawan.FindControl("ddlStatusPegawai")).Enabled = false;
				        }
				        else
                ...
                else if ((Session["GradeID"].ToString() == "3") || (Session["GradeID"].ToString() == "3a") || (Session["GradeID"].ToString() == "5") || (Session["GradeID"].ToString() == "6"))
                {
...
/*
				...
				try
				{
					...
					package = app.LoadPackage(@"C:\DataSQL\DTS\DataRekBank.dtsx", null);
					//Excute Package
...
/*
				  ...
				  pkgLocation =
					@"c:\datasql\dts\DataRekBank.dtsx";
				  app = new Application();
...
string strFilePath = "C:\\DataSQL\\DTS\\dts_norek2.bat";
...
psi.LoadUserProfile = true;
 psi.UserName = "rb";
 psi.Password = _password;
 psi.CreateNoWindow = true;
 psi.RedirectStandardOutput = true;
psi.RedirectStandardInput = true;
psi.RedirectStandardError = true;
psi.WorkingDirectory = "C:\\DataSQL\\DTS\\";
