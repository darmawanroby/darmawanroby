<%@ Page Language="C#" MasterPageFile="~/MasterPage.master" Title="Sistem Kehadiran Kerja Pegawai" AutoEventWireup="true" Inherits="rtwin.grant_department" CodeFile="grant_branch.aspx.cs" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="cphWithMenu" Runat="Server">
    ...
    <asp:UpdatePanel id="UpdatePanel1" runat="server">
        <contenttemplate>
<asp:Label id="ExceptionDetails" runat="server" CssClass="fontR" SkinID="fontR" EnableViewState="False"></asp:Label><br /><br /><asp:Label id="lblUser" runat="server"></asp:Label><br /><br /><table class="border_table" cellspacing=0 cellpadding=0 width="90%" border=0><tbody><tr><td style="WIDTH: 300px; HEIGHT: 13px" align=left><asp:GridView id="GvOtoritasCabang" runat="server" DataSourceID="dsOtoritasCabang" OnRowCommand="GvOtoritasCabang_RowCommand" OnRowDataBound="GvOtoritasCabang_RowDataBound" OnDataBound="GvOtoritasCabang_DataBound" AllowPaging="True" AllowSorting="True" AutoGenerateColumns="False" DataKeyNames="KODE_CABANG"><Columns>
...
<ItemTemplate>
<asp:Label id="Label1" runat="server" Text='<%# Bind("KODE_CABANG") %>' __designer:wfdid="w304"></asp:Label> 
</ItemTemplate>
</asp:TemplateField>
...
<ItemTemplate>
<asp:Label id="Label2" runat="server" Text='<%# Bind("NAMA_CABANG") %>' __designer:wfdid="w301"></asp:Label> 
</ItemTemplate>
</asp:TemplateField>
...
</asp:GridView><br /><asp:Button id="btnUpdate" onclick="btnUpdate_Click" runat="server" Text="<%$ Resources:imgUpdate %>"></asp:Button><br /><asp:Label id="lblKetAdmin" runat="server" Visible="False" CssClass="fontR" SkinID="fontR" Text="<%$ Resources:lblKetAdmin %>"></asp:Label><br /><asp:SqlDataSource id="dsOtoritasCabang" runat="server" ConnectionString="<%$ ConnectionStrings:strCn %>" SelectCommand="SELECT KODE_CABANG, NAMA_CABANG, HAK FROM q_OtoritasCabang  &#13;&#10;WHERE (USERNAME = @USERNAME) " DeleteCommand="DELETE FROM taOtoritasCabang WHERE USERNAME=@USERNAME AND KODE_CABANG=@KODE_CABANG" UpdateCommand="INSERT INTO taOtoritasCabang(USERNAME,KODE_CABANG) VALUES(@USERNAME,@KODE_CABANG)" OnDeleted="dsOtoritasCabang_Deleted" OnUpdated="dsOtoritasCabang_Updated">
                        <SelectParameters>
