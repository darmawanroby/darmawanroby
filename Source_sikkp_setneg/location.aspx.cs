...
namespace rtwin
	{
		public partial class location : BasePage
		{
            ...
            protected void dsLocation_Inserting(object sender, SqlDataSourceCommandEventArgs e)
            {
                ...
                if (GvInstitutions.Rows.Count > 0)
                {
                    ...
                    e.Command.Parameters["@KODE_LOKASI"].Value = NewKode.Text;
                    e.Command.Parameters["@NAMA_LOKASI"].Value = NewNamaInstansi.Text;
                }
            }

            //fungsi: menghandle ketika datasource telah selesai melakukan proses insert
            protected void dsLocation_Inserted(object sender, SqlDataSourceStatusEventArgs e)
            {
            ...
            protected void dsLocation_Deleted(object sender, SqlDataSourceStatusEventArgs e)
            {
                //seleksi apakah ada error (ditandai dengan jumlah baris yg terkena efek, 
                //jika = 0 / tidak ada yg terkena efek maka terjadi error)
                //jika tidak ada error / affectedRow > 0 maka lakukan penulisan log/userAct ke database
                ...
                {
                    //Tidak terjadi Error, lakukan penulisan log/userAct ke database
                    azlib.AddUserAct(Session["UserID"].ToString(), "1223", e.Command.Parameters["@KODE_LOKASI"].Value.ToString(), Application["strCn"].ToString());
                }
            }

            //fungsi: menghandle ketika datasource telah selesai melakukan proses update
            protected void dsLocation_Updated(object sender, SqlDataSourceStatusEventArgs e)
            {
                //seleksi apakah ada error (ditandai dengan jumlah baris yg terkena efek, 
                //jika = 0 / tidak ada yg terkena efek maka terjadi error)
                //jika tidak ada error / affectedRow > 0 maka lakukan penulisan log/userAct ke database
                ...
                {
                    //Tidak terjadi Error, lakukan penulisan log/userAct ke database
                    azlib.AddUserAct(Session["UserID"].ToString(), "1222", e.Command.Parameters["@KODE_LOKASI"].Value.ToString(), Application["strCn"].ToString());
                }
            }
