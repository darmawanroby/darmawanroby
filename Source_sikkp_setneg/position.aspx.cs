...
//        Nama file     : position.aspx.cs                             (Pseudo code)
//        File terkait  : position.aspx                                (Form)
//                        \App_LocalResources\position.aspx.id.resx    (Display label bahasa Indonesia)
//                        \App_LocalResources\position.aspx.en.resx    (Display label bahasa Inggris)
//                        \App_Code\azlib.cs                         (Kumpulan fungsi-fungsi)
//
//
//        Deskripsi     : Halaman untuk menampilkan dan mengolah tabel Jabatan (taJabatan)
//
// 2. Penulis Program
...
namespace rtwin
	{
		public partial class position : BasePage
		{
            ...
            {
                GvPosition.Visible = show;
                FvPosition.Visible = !show;
                if (show)
                {
                    FvPosition.DefaultMode = FormViewMode.ReadOnly;
                }
            }
            ...
            protected void Page_Load(object sender, System.EventArgs e)
			{
                ...
                else
                {
                    ...
                    if ((!Page.IsPostBack) && (Session["GradeID"].ToString() != "1"))
                    {
                        ...
                        GvPosition.Columns[2].Visible = false;
                        GvPosition.Columns[3].Visible = false;
                    }
                }
			}
			
            //fungsi: mengatur state Gridview setelah berhasil bound data
			protected void GvPosition_DataBound(object sender, System.EventArgs e)
			{
                //seleksi jumlah data kosong atau tidak
                if (GvPosition.Rows.Count > 0)
                {
                    //Data tidak kosong
                    GridViewRow rowPager = GvPosition.BottomPagerRow;
                    ...
                    if (rowPager != null)
                    {
                        ...
                        {
                            GvPosition.PageIndex = GvPosition.PageCount - 1;
                        }
                        ...
                        txtHalaman.Text = Convert.ToString(GvPosition.PageIndex + 1);
                        lblJumlahHalaman.Text = GvPosition.PageCount.ToString();
                    }
                    showGridView(true);
                }
                else
            ...
            protected void GvPosition_RowEditing(object sender, GridViewEditEventArgs e)
            {
                if (Page.IsValid)
                {
                    ...
                    GvPosition.DataBind();
                    this.ScriptManager1.SetFocus(GvPosition.Rows[e.NewEditIndex].Cells[1].FindControl("txtCol1Edit").ClientID);
                }
                else
                {
                    e.Cancel = true;
            ...
            protected void lnkBtnInsert_Click(object sender, EventArgs e)
            {
                if (Page.IsValid)
                {
                    ...
                    if (GvPosition.Rows.Count > 0)
                    {
                        //Data tidak kosong
                        sendToLastPage = true;
                        GvPosition.ShowFooter = true;
                        this.ScriptManager1.SetFocus(GvPosition.FooterRow.Cells[0].FindControl("txtCol0Footer").ClientID);
                    }
                    else
                    {
                        //Data Kosong
                        FvPosition.DefaultMode = FormViewMode.Insert;
                        this.ScriptManager1.SetFocus(FvPosition.FindControl("txtCol0FormView"));
                    }
                }
            }
            ...
            protected void FvPosition_ItemCommand(object sender, FormViewCommandEventArgs e)
            {
            ...
            protected void dsJabatan_Inserting(object sender, SqlDataSourceCommandEventArgs e)
            {
                ...
                if (GvPosition.Rows.Count > 0)
                {
                    TextBox NewKode = (TextBox)GvPosition.FooterRow.FindControl("txtCol0Footer");
                    TextBox NewNamaJabatan = (TextBox)GvPosition.FooterRow.FindControl("txtCol1Footer");

                    e.Command.Parameters["@kode_jabatan"].Value = NewKode.Text;
                    e.Command.Parameters["@nama_jabatan"].Value = NewNamaJabatan.Text;
                }
            }

            //fungsi: menghandle ketika datasource telah selesai melakukan proses insert
            protected void dsJabatan_Inserted(object sender, SqlDataSourceStatusEventArgs e)
            {
                ...
                else
                {
                    ...
                    azlib.AddUserAct(Session["UserID"].ToString(), "1281", e.Command.Parameters["@KODE_JABATAN"].Value.ToString(), Application["strCn"].ToString());
                }
                GvPosition.ShowFooter = false;
                showGridView(true);
            }
            ...
            protected void dsJabatan_Deleted(object sender, SqlDataSourceStatusEventArgs e)
            {
                //seleksi apakah ada error (ditandai dengan jumlah baris yg terkena efek, 
                //jika = 0 / tidak ada yg terkena efek maka terjadi error)
                //jika tidak ada error / affectedRow > 0 maka lakukan penulisan log/userAct ke database
                ...
                {
                    //Tidak terjadi Error, lakukan penulisan log/userAct ke database
                    azlib.AddUserAct(Session["UserID"].ToString(), "1283", e.Command.Parameters["@KODE_JABATAN"].Value.ToString(), Application["strCn"].ToString());
                }
            }

            //fungsi: menghandle ketika datasource telah selesai melakukan proses update
            protected void dsJabatan_Updated(object sender, SqlDataSourceStatusEventArgs e)
            {
                //seleksi apakah ada error (ditandai dengan jumlah baris yg terkena efek, 
                //jika = 0 / tidak ada yg terkena efek maka terjadi error)
                //jika tidak ada error / affectedRow > 0 maka lakukan penulisan log/userAct ke database
                ...
                {
                    //Tidak terjadi Error, lakukan penulisan log/userAct ke database
                    azlib.AddUserAct(Session["UserID"].ToString(), "1282", e.Command.Parameters["@KODE_JABATAN"].Value.ToString(), Application["strCn"].ToString());
                }
            }
            ...
            protected void txtHalaman_TextChanged(object sender, System.EventArgs e)
            {
                ...
                try
                {
                    if (int.Parse(txtHalaman.Text) <= GvPosition.PageCount + 1 && int.Parse(txtHalaman.Text) > 0)
                    {
                        GvPosition.PageIndex = (int.Parse(txtHalaman.Text)) - 1;
                        GvPosition.DataBind();
                    }                    
                }
                catch (Exception)
                {
                    txtHalaman.Text = (GvPosition.PageIndex + 1).ToString();
                }
            }
}
}
